 ...
 * MyBB: MySupport
 *
 * File: inc/plugins/mysupport/mysupport.php
 * 
 * Authors: Matthew Rogowski (1.8 usage edits by Vintagedaddyo)
 *
 * MyBB Version: 1.8
 *
 * Plugin Version: 0.5
...
function mysupport_do_info()
{
    ...
    $lang->mysupport_info_description = '<form action="https://www.paypal.com/cgi-bin/webscr" method="post" style="float:right;">' .
        '<input type="hidden" name="cmd" value="_s-xclick">' . 
        '<input type="hidden" name="hosted_button_id" value="AZE6ZNZPBPVUL">' .
        '<input type="image" src="https://www.paypalobjects.com/en_US/i/btn/btn_donate_SM.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!">' .
        '<img alt="" border="0" src="https://www.paypalobjects.com/pl_PL/i/scr/pixel.gif" width="1" height="1">' .
        '</form>' . $lang->mysupport_info_description;


	return array(
		'name' => $lang->mysupport_info_name,
		'description' => $lang->mysupport_info_description,
		'website' => 'https://github.com/vintagedaddyo/MyBB_Plugin-MySupport',
		'author' => 'MattRogowski & updated by Vintagedaddyo',
	...
	if(!$db->table_exists("mysupport"))
	{
		$db->write_query("
			CREATE TABLE  " . TABLE_PREFIX . "mysupport (
				...
				`extra` VARCHAR(255) NOT NULL DEFAULT 'default value'
			) ENGINE = MYISAM ;
		");
	}
...
<form action="{$mybb->settings[\'bburl\']}/showthread.php" method="post">
	...
	<table border="0" cellspacing="{$theme[\'borderwidth\']}" cellpadding="{$theme[\'tablespace\']}" class="tborder">
		...
		$templates[] = array(
			...
			"template" => " <a href=\"{\$mybb->settings['bburl']}/showthread.php?action=bestanswer&amp;pid={\$post['pid']}&amp;my_post_key={\$mybb->post_code}\"><img src=\"{\$theme['imgdir']}/{\$bestanswer_img}.gif\" alt=\"{\$bestanswer_alt}\" title=\"{\$bestanswer_title}\" /> {\$bestanswer_desc}</a>"
		);
		$templates[] = array(
	...
	<td class=\"{\$bgcolor}\" width=\"25%\">{\$thread['forumlink']} <a href=\"{\$mybb->settings['bburl']}/{\$view_all_forum_link}\"><img src=\"{\$theme['imgdir']}/mysupport_arrow_right.gif\" alt=\"{\$view_all_forum_text}\" title=\"{\$view_all_forum_text}\" /></a></td>
...
<table border=\"0\" cellspacing=\"{\$theme['borderwidth']}\" cellpadding=\"{\$theme['tablespace']}\" class=\"tborder\">
	...
	<tr>
		<td class=\"trow2\" width=\"100%\">
			<table border=\"0\" cellspacing=\"{\$theme['borderwidth']}\" cellpadding=\"{\$theme['tablespace']}\" class=\"tborder\" style=\"border-radius:0px!important;\">
				<tr>
					{\$solved_row}
					{\$notsolved_row}
					{\$technical_row}
...
function mysupport_template_edits($type)
{
	...
	if($type == 1)
	{
		...
		find_replace_templatesets("postbit", "#".preg_quote('post_controls')."#i", 'post_controls{$post[\'mysupport_bestanswer_highlight\']}{$post[\'mysupport_staff_highlight\']}');
		...
		find_replace_templatesets("postbit_classic", "#".preg_quote('post_controls')."#i", 'post_controls{$post[\'mysupport_bestanswer_highlight\']}{$post[\'mysupport_staff_highlight\']}');
		...
		find_replace_templatesets("showthread", "#".preg_quote('{$thread[\'displayprefix\']}')."#i", '{$mysupport_status}{$thread[\'displayprefix\']}');
		find_replace_templatesets("header", "#".preg_quote('{$pending_joinrequests}')."#i", '{$pending_joinrequests}{$mysupport_tech_notice}{$mysupport_assign_notice}');
		...
		find_replace_templatesets("modcp_nav_modlogs", "#".preg_quote('{$lang->mcp_nav_modlogs}</a></td></tr>')."#i", '{$lang->mcp_nav_modlogs}</a></td></tr>{mysupport_nav_option}');
	...
	else
	{
		...
		find_replace_templatesets("modcp_nav_modlogs", "#".preg_quote('{mysupport_nav_option}')."#i", '', 0);
